...
<div class="container">
	...
	<div class="row text-center" style="margin-top: 65px">
		<h1>CREST Reasoning Olympiad (CRO) - Cut-off</h1>
	</div>
...
<thead>
  <tr>
      <th>Class</th>
      <th>Cut-off For Medals</th>
      <th>Cut-off For Merit Certificate</th> 
  </tr>
</thead>

<tbody>
	 <?php
foreach($crorescutoff as $rows) {
	//foreach ($value as $rows) {
		# code...
	
	 ?>
	<tr class="<?php echo $rows['class']%2?'warning':'danger' ?>">
      <td style="text-align: center;"><?=$rows['class'];?></td>
      <td style="text-align: center;"><?=$rows['cut_off_for_medal'].'%';?></td>
      <td style="text-align: center;"><?=$rows['cut_off_for_merit_certificate'].'%';?></td>
  </tr>
