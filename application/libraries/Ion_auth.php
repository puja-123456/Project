...
class Ion_auth {
    ...
    public function __construct() {
        ...
        // load the email data from database
        // $email_data = $this->base_model->run_query("select * from " . $this->db->dbprefix('email_setting') . " limit 1");
        // if (count($email_data) > 0) {
        //     $email_data = $email_data[0];
        //     $email_config['smtp_host'] = $email_data->smtp_host;
        //     $email_config['smtp_user'] = $email_data->smtp_user;
        //     $email_config['smtp_pass'] = $email_data->smtp_pass;
        //     $email_config['smtp_port'] = $email_data->smtp_port;
        // }
    ...
    public function register($username, $password, $email, $additional_data = array(), $group_ids = array()) { //need to test email activation
        ...
        if (!$email_activation) {
            ...
            $deactivate = $this->ion_auth_model->deactivate($id, '1');
            ...
            if (!$this->config->item('use_ci_email', 'ion_auth')) {
                ...
                $config = Array(
                    ...
                    'smtp_user' => 'smartexam4u@gmail.com',
                    'smtp_pass' => 'prabhakar@123',
                    'smtp_port' => '465',
                    'charset' => 'utf-8',
                    'newline' => '\r\n',
                    'mailtype' => 'html',
    ...
    public function logout() {
        ...
        $identity = $this->config->item('identity', 'ion_auth');
        $this->session->unset_userdata(array($identity => '', 'id' => '', 'user_id' => '', 'username' => '', 'email' => '', 'fb_login' => ''));
    ...
    public function is_moderator($id = false) {
        ...
        $moderator_group = $this->config->item('moderator_group', 'ion_auth');

        return $this->in_group($moderator_group, $id);
    }
