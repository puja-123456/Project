...
;(function ($, document, window){
	...
	attachEventsHandler = function(){
		...
		that.$element.find('.steps > a').on('click', function(e){
			e.stopPropagation();
	...
	checkStatus = function(){
		var that = this,
			...
			stepsItems = that.$element.find('.steps > a'),
			opts = that.options;
			var value=$('div.active').attr('data-step');
           // alert(value);
/*     
			that.$element.find('.btn-next, .btn-prev').on('click', function(e){
			//if($(this).attr('disabled') || $(this).hasClass('disabled') || !$(this).is(':visible')) return;
			...
			$(".btn-prev span").click(function(){
 
	
      //fire your ajax call  
     $("input[name=action]").val("previous");
    //$(this).val("previous");
    //$("#action").val("previous");
    	//e.preventDefault();
   // e.preventDefault();
...
//$("#saction").val("previous");
 //$('html,body').animate({scrollTop: $(".quction").offset().top + 120},'slow');
});



			$(".next-text span").click(function(){
...
$('html,body').animate({scrollTop: $(".quction").offset().top});
...
$('html,body').animate({scrollTop: $(".quction").offset().top});
			...
			// var prev = $("input[name=action]").val();
...
// alert(prev);
			...
			{
				this.currentStep=parseInt(value)-1;
			}
		...
		that.$element
			.find('.steps > a[data-step="'+ that.currentStep +'"]')
			.addClass('active');
	...
	moveStep = function(step, direction, event, checkStep){		
		...
		steps = that.$element.find('.steps > a'),
		triggerEnd = false;
	...
	methods = {
		init: function (element, options) {
			...
			var stepsLi = this.$element.find('.steps > a');
			...
			this.$element.find('.steps > a:last-child').addClass('final');
